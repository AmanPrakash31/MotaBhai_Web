...
const TestimonialRowImage = ({ src, alt }) => {
...
export default function AdminDashboard() {
  ...
  const handleAction = async (formAction, formData, successMessage) => {
  ...
  const onMotorcycleSubmit = (values) => {
  ...
  const onTestimonialSubmit = (values) => {
    const formData = new FormData();
     Object.entries(values).forEach(([key, value]) => {
  ...
  const openModal = (type, mode, data) => {
    ...
    if (type === 'motorcycle') {
        ...
        motorcycleForm.reset(defaultValues);
    }
    if (type === 'testimonial') {
        const defaultValues = mode === 'edit' && data ? { ...data, existingImage: data.image || undefined, image: undefined } : { rating: 5, existingImage: undefined, image: undefined };
        testimonialForm.reset(defaultValues);
    }
  };

  const handleRemoveImage = (imageUrl, fieldName) => {
